...
// Stores the lead in Supabase (server-side, service-role key) and emails info@stack360.ai via Resend.
// No npm dependencies: uses built-in fetch (Node 18+ on Vercel).
...
export default async function handler(req, res){
  ...
  try {
    ...
    // Honeypot: real users never fill this; bots do. Accept silently, store nothing.
    ...
    // 1) Store in Supabase (service-role key = server-side only, bypasses RLS)
    ...
    // 2) Notify via Resend (optional but recommended)
    if (process.env.RESEND_API_KEY){
      await fetch('https://api.resend.com/emails', {
        method:'POST',
        headers:{ Authorization:`Bearer ${process.env.RESEND_API_KEY}`, 'Content-Type':'application/json' },
        body: JSON.stringify({
          from:'Stack360 <notifications@stack360.ai>',
          to:['info@stack360.ai'],
          reply_to: email,
          subject:`New Founding Partner enquiry — ${company}`,
          text:`Name: ${name}\nFirm: ${company}\nTitle: ${title}\nLocation: ${location}\nPhone: ${phone}\nWork email: ${email}\nSource: ${row.source}`
        })
      }).catch(e => console.error('resend error', e)); // don't fail the request if email hiccups; lead is already stored
    }
